...
import { isEntryLocked, submitEntry, finalizeEntries, ensureSchedule, openEntryWindow } from '../../sim/tournament';
  ...
  return (
    <section>
      <div style={{display:'flex',alignItems:'center',gap:12}}>
        ...
        {!locked && <small>Max 16 entrants; oversub trims by record+fame</small>}
      ...
      {!locked && (
        <div style={{display:'flex', gap:8, marginTop:8}}>
          ...
          <button onClick={()=>{
            ...
            submitEntry(division, candidates[0].id);
          }}>Submit first match</button>
        </div>
      )}
      <div style={{marginTop:8}}>
        <b>Current Entries ({entries.length}/16)</b>
        <ul>
          {entries.map(w=> <li key={w.id}>{w.name}</li>)}
          {!entries.length && <li className="muted">No entries yet.</li>}
        </ul>
      </div>
    </section>
  );
